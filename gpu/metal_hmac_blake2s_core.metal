/*
 * $Revision: 1.1 $
 * $Log: metal_hmac_blake2s_core.metal,v $
 * Revision 1.1  2026/05/16 20:00:43  dlr
 * Initial codegen output -- cl2metal.py from gpu/gpu_hmac_blake2s_core.metal.cl with overlay codegen/cl2metal_overrides/hmac_blake2s_core.metal.yaml
 *
 *
 * Auto-generated by gpu/codegen/cl2metal.py from
 *   gpu_hmac_blake2s_core.cl (RCS rev 1.3).
 *
...
{
    for (int i = 0; i < 8; i++) st.h[i] = MTL_B2S_IV[i];
...
static inline void hmac_blake2s_run(thread uint *out,
                                thread const uchar *data,
                                int datalen)
{
    uint h[8];
    for (int i = 0; i < 8; i++) h[i] = MTL_B2S_IV[i];
    h[0] ^= 0x01010020u;
