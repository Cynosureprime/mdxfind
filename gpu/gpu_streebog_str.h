/* Auto-generated from gpu_streebog.cl -- do not edit */
static const char gpu_streebog_str[] =
    ...
    "void streebog_hash_global(uchar *out, int hlen, __global const uchar *data, int datalen) {\n"
    "    ulong h[8], e[8], m[8];\n"
    "    ulong n = 0;\n"
    "    int c;\n"
    "\n"
    ...
    "__kernel void streebog256_unsalted_batch(\n"
    ...
    "\n"
    "    uchar hash[64];  /* big enough for hex iteration input too */\n"
    ...
    "        uint h[8];\n"
    "        for (int i = 0; i < 8; i++)\n"
    "            h[i] = ((uint)hash[i*4]) | ((uint)hash[i*4+1] << 8)\n"
    ...
    "        }\n"
    "        if (iter < max_iter) {\n"
    "            /* Hex-encode 32 hash bytes into hex_str[64] (64 hex chars, lowercase) */\n"
    "            uchar hex_str[64];\n"
    "            for (int i = 0; i < 32; i++) {\n"
    "                uint b = hash[i];\n"
    "                uint hi = (b >> 4) & 0xf, lo = b & 0xf;\n"
    "                hex_str[i*2]   = hi + ((hi < 10) ? '0' : ('a'-10));\n"
    "                hex_str[i*2+1] = lo + ((lo < 10) ? '0' : ('a'-10));\n"
    "            }\n"
    "            streebog_hash_priv(hash, 32, hex_str, 64);\n"
    "        }\n"
    "    }\n"
    "}\n"
    "\n"
    "/* ==== Unsalted Streebog-512 (e431) ==== */\n"
    "__kernel void streebog512_unsalted_batch(\n"
    "    __global const uint *words, __global const ushort *unused_lens,\n"
    "    __global const uchar *mask_desc,\n"
    "    __global const uint *unused1, __global const ushort *unused2,\n"
    "    __global const uint *compact_fp, __global const uint *compact_idx,\n"
    ...
    "    for (int i = 0; i < passlen && i < 55; i++) pass[i] = mb[i];\n"
    "\n"
    "    uchar hash[128];  /* big enough for hex iteration input too */\n"
    ...
    "                          hash_data_buf, hash_data_off,\n"
    "                          overflow_keys, overflow_hashes, overflow_offsets, params.overflow_count)) {\n"
    "            EMIT_HIT_16(hits, hit_count, params.max_hits, word_idx, mask_idx, iter, h)\n"
    "        }\n"
    "        if (iter < max_iter) {\n"
    "            /* Hex-encode 64 hash bytes into hex_str[128] (128 hex chars, lowercase) */\n"
    "            uchar hex_str[128];\n"
    "            for (int i = 0; i < 64; i++) {\n"
    "                uint b = hash[i];\n"
    "                uint hi = (b >> 4) & 0xf, lo = b & 0xf;\n"
    ...
    "                hex_str[i*2+1] = lo + ((lo < 10) ? '0' : ('a'-10));\n"
    "            }\n"
    "            streebog_hash_priv(hash, 64, hex_str, 128);\n"
    "        }\n"
    "    }\n"
    "}\n"
    "\n"
    "/* ==== HMAC-Streebog-256 key=$pass (e837, hashcat 11750) ==== */\n"
    "__kernel void hmac_streebog256_kpass_batch(\n"
    ...
    "__kernel void hmac_streebog256_ksalt_batch(\n"
    "    __global const uchar *hexhashes, __global const ushort *hexlens,\n"
    "    __global const uchar *salts, __global const uint *salt_offsets, __global const ushort *salt_lens,\n"
    "    __global const uint *compact_fp, __global const uint *compact_idx,\n"
    "    __global const OCLParams *params_buf,\n"
    ...
    "__kernel void hmac_streebog512_kpass_batch(\n"
    "    __global const uchar *hexhashes, __global const ushort *hexlens,\n"
    "    __global const uchar *salts, __global const uint *salt_offsets, __global const ushort *salt_lens,\n"
    "    __global const uint *compact_fp, __global const uint *compact_idx,\n"
    "    __global const OCLParams *params_buf,\n"
    ...
    "__kernel void hmac_streebog512_ksalt_batch(\n"
    "    __global const uchar *hexhashes, __global const ushort *hexlens,\n"
    "    __global const uchar *salts, __global const uint *salt_offsets, __global const ushort *salt_lens,\n"
    "    __global const uint *compact_fp, __global const uint *compact_idx,\n"
    "    __global const OCLParams *params_buf,\n"
