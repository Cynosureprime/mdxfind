 ...
 *
 * NOTE: identifiers WRL_SBOX, WRL_RC, WRL_OP, whirlpool_compress* are
 * locally defined in this translation unit. The slab kernel
 * gpu_wrlunsalted.cl carries its own identical definitions, but each
 * kernel is built into a separate cl_program (different sources passed
 * to clCreateProgramWithSource). No name collision at link time.
 */
...
/* Whirlpool round constants (10 rounds). Identical to gpu_wrlunsalted.cl:17. */
...
/* Whirlpool S-Box: 8 sub-tables of 256 uint64 entries each. 16 KB total.
 *
 * DIAGNOSTIC NOTE: kept in __constant memory and indexed via shift-then-
 * mask (the WRL_OP macro below). This is structurally distinct from
 * Streebog's SBOG_LPS macro at gpu_streebog.cl:559-566 which casts the
 * ulong source to (uchar *) and reads byte-stride into the SBOB_SL64
 * table. If gfx1201 corrupts only the byte-pointer pattern, this
 * direct-ulong access pattern should pass.
 *
 * The contents below are byte-for-byte identical to gpu_wrlunsalted.cl
 * lines 25-555. */
...
/* Whirlpool WHIRLPOOL_OP macro equivalent. Direct ulong indexing of the
 * __constant table — `(src)[i] >> shift` is a 64-bit shift on the
 * private ulong register, then masked with 0xff. NO uchar pointer cast.
 * This is the diagnostic divergence vs Streebog's SBOG_LPS. */
