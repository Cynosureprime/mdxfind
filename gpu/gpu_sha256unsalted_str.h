...
static const char gpu_sha256unsalted_str[] =
    ...
    "void sha224_to_hex_lc(uint *state, uint *M) {\n"
    "    for (int i = 0; i < 7; i++) {\n"
    "        uint s = state[i];\n"
    "        uint b0 = (s >> 24) & 0xff, b1 = (s >> 16) & 0xff;\n"
    "        uint b2 = (s >> 8)  & 0xff, b3 = s & 0xff;\n"
    ...
    "                          hash_data_buf, hash_data_off,\n"
    "                          overflow_keys, overflow_hashes, overflow_offsets, params.overflow_count)) {\n"
    "            EMIT_HIT_7(hits, hit_count, params.max_hits, word_idx, mask_idx, iter, h)\n"
    "        }\n"
    "        if (iter < max_iter) {\n"
    "            /* Hex-encode 7 BE state words into M[0..13] (56 hex chars).\n"
