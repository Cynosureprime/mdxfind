...
/* Rule-walker scratch, in ELEMENTS not bytes.
 *
 * The host INJECTS -D RULE_BUF_MAX from the single constant
 * GPU_RULES_WALKER_BUF_ELEMS in gpujob.h (= 2 x the input-word gate), so the
 * value below is only a fallback for out-of-band compiles -- gpu_rules_test.c,
 * or a standalone clang/xcrun syntax check.  OpenCL injects at the one funnel
 * in gpu_kernel_cache.c and folds it into the cache key; Metal injects via
 * metal_compile_opts() in gpu_metal.m.
 *
 * ELEMENTS on purpose: this walker has 1 element = 1 uchar, and a future
 * UTF-32 walker keeps the element count at 4x the bytes, so the constant does
 * not need re-deriving for that port.
 *
 * RULE_BUF_LIMIT is 15 BELOW the allocation, not 1: it covers a NUL and the
 * n+1 boundary conditions in the per-op bounds checks.
 *
 * Exceeding the limit is DEFINED behaviour, not an error -- the rule does what
 * it can, stops, and whatever was generated becomes the candidate.  The CPU
 * engine does the same at MAXLINE.  The two limits differ, so GPU and CPU
 * candidates diverge above this one; `-G none` is the answer when CPU-exact
 * output is required.  Nothing is checked about what a rule can PRODUCE; the
 * only length check anywhere is the input-word gate.
 */
#ifndef RULE_BUF_MAX
