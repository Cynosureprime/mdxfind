 ...
 *
 */
/* gpu_keccak384_core.cl - Keccak-384 algorithm extension functions for the
 ...
 * domain suffix=0x01 (plain Keccak). HASH_WORDS=12 = 12 uint32 LE; the
 * underlying ulong state holds 6 ulong = 48 bytes. HASH_BLOCK_BYTES=104 = rate.
 *
 * State decomposition for h[]:
 *   h[2i]   = (uint)sp[i]
 *   h[2i+1] = (uint)(sp[i] >> 32)   for i = 0..5
 *
 * Source order: [ gpu_common_str, gpu_md5_rules_str, gpu_keccak384_core_str,
 *   gpu_template_str ]. gpu_common_str provides keccakf1600() (rev 1.14+).
 *
 * Bytecast invariant: sp[0..5] LE byte image == sph_keccak384_close output
 * (mdxfind.c JOB_KECCAK384). gpu_hash_words(JOB_KECCAK384) returns 12.
 */
...
static inline void template_state_to_h_keccak384(template_state *st) {
...
static inline void keccak384_absorb_pad(ulong *sp, const uchar *data, int rem) {
...
static inline void keccak384_absorb_full(ulong *sp, const uchar *data) {
    for (int i = 0; i < KECCAK_RATE_WORDS; i++) {
        ulong w = 0;
        int b = i * 8;
        w |= (ulong)data[b];
...
static inline void template_transform(template_state *st, const uchar *block) {
    keccak384_absorb_full(&st->sp[0], block);
}
                                     ...
                                     const uchar *data, int len)
{
    ...
    while ((len - pos) >= KECCAK_RATE) {
        keccak384_absorb_full(&st->sp[0], data + pos);
        pos += KECCAK_RATE;
    }
    int rem = len - pos;
    keccak384_absorb_pad(&st->sp[0], data + pos, rem);
    template_state_to_h_keccak384(st);
}
...
static inline void template_iterate(template_state *st)
{
    ...
    /* hex_lc: 12 words * 8 chars = 96 chars = 2 * 48 bytes. 96 < rate=104,
     * so single absorb_pad call (no full block). */
    ...
    while ((hex_chars - pos) >= KECCAK_RATE) {
        keccak384_absorb_full(&st->sp[0], hex + pos);
        pos += KECCAK_RATE;
    }
    int rem = hex_chars - pos;
    keccak384_absorb_pad(&st->sp[0], hex + pos, rem);
    template_state_to_h_keccak384(st);
}
