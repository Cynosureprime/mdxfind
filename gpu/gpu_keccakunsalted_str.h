...
static const char gpu_keccakunsalted_str[] =
    ...
    "  KECCAK_KERNEL_PAD(name, rate, rate_words, hash_words, hash_ulongs, 0x01)\n"
    "#define SHA3_KERNEL(name, rate, rate_words, hash_words, hash_ulongs)           \\\n"
    "  KECCAK_KERNEL_PAD(name, rate, rate_words, hash_words, hash_ulongs, 0x06)\n"
    "#define KECCAK_KERNEL_PAD(name, rate, rate_words, hash_words, hash_ulongs, kpad_byte) \\\n"
    "__kernel void name(                                                            \\\n"
    ...
    "                hits[_base] = word_idx; hits[_base+1] = mask_idx;              \\\n"
    "                hits[_base+2] = iter;                                          \\\n"
    "                for (int i = 0; i < hash_words; i++) hits[_base+3+i] = h[i];   \\\n"
    "                for (uint _z = 3 + hash_words; _z < HIT_STRIDE; _z++)          \\\n"
    "                    hits[_base+_z] = 0;                                        \\\n"
    ...
    "            int hex_bytes = hash_ulongs * 8;  /* actual hash byte count */     \\\n"
    "            if (hash_words == 7) hex_bytes = 28;  /* SHA224/Keccak224 */       \\\n"
    "            if (hash_words == 12) hex_bytes = 48; /* SHA384/Keccak384 */       \\\n"
    "            int hex_chars = hex_bytes * 2;                                     \\\n"
    "            uchar hex_str[256];                                                \\\n"
    "            for (int i = 0; i < hex_bytes; i++) {                              \\\n"
    "                uchar b = (uchar)(st[i/8] >> ((i%8)*8));                       \\\n"
    "                uchar hi = (b >> 4), lo = (b & 0xf);                          \\\n"
    "                hex_str[i*2]   = hi + (hi < 10 ? '0' : 'a' - 10);            \\\n"
    "                hex_str[i*2+1] = lo + (lo < 10 ? '0' : 'a' - 10);            \\\n"
    "            }                                                                  \\\n"
    "            /* Absorb hex string with sponge padding into fresh state */       \\\n"
    "            for (int i = 0; i < 25; i++) st[i] = 0;                           \\\n"
    "            /* XOR hex chars into state as LE bytes */                         \\\n"
    ...
    "            }                                                                  \\\n"
    "            if (hex_chars < rate) {                                            \\\n"
    "                /* Fits in one block: apply padding in same block */           \\\n"
    "                int pos = hex_chars;                                           \\\n"
    "                st[pos/8] ^= (ulong)pad_byte << ((pos%8)*8);                  \\\n"
    "                st[(rate-1)/8] ^= 0x80UL << (((rate-1)%8)*8);                 \\\n"
    "                keccak_f1600(st);                                              \\\n"
    "            } else {                                                           \\\n"
    "                /* Multi-block: first block is full rate bytes, no padding */  \\\n"
    "                keccak_f1600(st);                                              \\\n"
    "                /* Remaining hex chars in second block */                      \\\n"
    "                for (int i = rate; i < hex_chars; i++) {                       \\\n"
    "                    int wi = (i - rate) / 8;                                   \\\n"
    "                    int bi = ((i - rate) % 8) * 8;                             \\\n"
    "                    st[wi] ^= (ulong)hex_str[i] << bi;                        \\\n"
    "                }                                                              \\\n"
    "                int pos2 = hex_chars - rate;                                   \\\n"
    "                st[pos2/8] ^= (ulong)pad_byte << ((pos2%8)*8);                \\\n"
    "                st[(rate-1)/8] ^= 0x80UL << (((rate-1)%8)*8);                 \\\n"
    "                keccak_f1600(st);                                              \\\n"
    "            }                                                                  \\\n"
    "        }                                                                      \\\n"
    ...
    "SHA3_KERNEL(sha3_224_unsalted_batch, 144, 18, 7, 4)\n"
    "SHA3_KERNEL(sha3_256_unsalted_batch, 136, 17, 8, 4)\n"
    "SHA3_KERNEL(sha3_384_unsalted_batch, 104, 13, 12, 6)\n"
    "SHA3_KERNEL(sha3_512_unsalted_batch, 72, 9, 16, 8)\n"
;
