...
static const char gpu_md6256unsalted_str[] =
    ...
    "        }\n"
    "        if (iter < max_iter) {\n"
    "            /* Hex-encode 32-byte hash (4 × BE uint64) into 64-char hex string.\n"
    "             * Build workspace buffer, then repack into N[89] for next iteration. */\n"
    "            uchar hex_str[64];\n"
    "            for (int i = 0; i < 4; i++) {\n"
    "                ulong w = C[i];  /* already BE from compression */\n"
    "                for (int b = 0; b < 8; b++) {\n"
    "                    uchar byte = (uchar)(w >> (56 - b * 8));\n"
    "                    uchar hi = (byte >> 4), lo = (byte & 0xf);\n"
    "                    hex_str[i*16 + b*2]     = hi + (hi < 10 ? '0' : 'a' - 10);\n"
    "                    hex_str[i*16 + b*2 + 1] = lo + (lo < 10 ? '0' : 'a' - 10);\n"
    "                }\n"
    "            }\n"
    "\n"
    ...
    "            /* A[23] = U = nodeID(ell=1, i=0) */\n"
    "            A[23] = (ulong)1 << 56;\n"
    "            /* A[24] = V = control word: r=104, L=64, z=1, p=pad_bits, keylen=0, d=256 */\n"
    "            int p_bits = 64 * 64 - 64 * 8;  /* 64 B-bytes, 64 hex chars used */\n"
    "            A[24] = ((ulong)104 << 48) | ((ulong)64 << 40) | ((ulong)1 << 36)\n"
    "                  | ((ulong)p_bits << 20) | (ulong)256;\n"
    "            /* A[25..88] = B: hex string in LE bytes, then pad with zeros */\n"
    "            for (int i = 25; i < 89; i++) A[i] = 0;\n"
    "            for (int i = 0; i < 64; i++) {\n"
    "                int wi = 25 + (i >> 3);\n"
    "                int bi = (i & 7) << 3;\n"
    "                A[wi] |= (ulong)hex_str[i] << bi;\n"
    "            }\n"
    "            /* Byte-swap B to BE for MD6 compression */\n"
    "            for (int i = 25; i < 89; i++) A[i] = bswap64(A[i]);\n"
    "\n"
    "            /* Run MD6 compression */\n"
    "            md6_compress_loop(A);\n"
