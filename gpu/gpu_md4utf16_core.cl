 ...
 * Revision 1.4  2026/09/17 03:39:46  dlr
 * Record why this core carries no ALT_DIGEST hook while its NTLMH sibling does: the MD4UTF16 CPU reference has no zero-extend arm, so a second digest would be one the CPU never computes. CORRECTION to the revision 1.3 message, which was wrong on two points: 1.3 REPLACED zero-extend with the real iconv conversion rather than adding it alongside, and it used no ALT_DIGEST hook. The code in 1.3 was right; only its description was wrong.
 *
 * Revision 1.3  2026/09/17 03:37:43  dlr
 * MD4UTF16 gains the same real-iconv UTF-16LE variant alongside zero-extend, via the ALT_DIGEST hook. Zero-extend and iconv agree only for ASCII; they diverge on any multi-byte UTF-8 input, which is the whole point of carrying both.
 *
 ...
 * reference tests BOTH an iconv arm and a zero-extend arm, so the GPU has
 * to compute and compare two digests or leave one arm unreachable.  The
 * MD4UTF16 CPU reference has NO zero-extend arm at all, so a second digest
 * would be one the CPU never computes -- this core computes the iconv form
