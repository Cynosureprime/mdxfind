/* Auto-generated from metal_streebog.metal -- do not edit */
static const char metal_streebog_str[] =
    ...
    "};\n"
    "\n"
    "/* gpu_streebog.cl -- GOST R 34.11-2012 (Streebog) GPU kernels\n"
    " * Unsalted: streebog256 (e430), streebog512 (e431)\n"
    " * HMAC key=$pass: e837 (256-KPASS/11750), e839 (512-KPASS/11850)\n"
    " * HMAC key=$salt: e838 (256/11760), e840 (512/11860)\n"
    " *\n"
    " * 64-byte block, big-endian byte order (reversed from typical LE).\n"
    " * Uses sbob_sl64[8][256] (16KB) + sbob_rc64[12][8] S-box tables.\n"
    " */\n"
    "\n"
    "/* ---- Charset tables for mask expansion (must precede kernels) ---- */\n"
    "\n"
    "/* ---- LPS transform macro ---- */\n"
    ...
    "                    SBOB_SL64[4][tb[_i+32]] ^ SBOB_SL64[5][tb[_i+40]] ^ \\\n"
    "                    SBOB_SL64[6][tb[_i+48]] ^ SBOB_SL64[7][tb[_i+56]]; \\\n"
    "} while(0)\n"
    "\n"
    ...
    "    *hw = ((uint)hash[12]) | ((uint)hash[13]<<8) | ((uint)hash[14]<<16) | ((uint)hash[15]<<24);\n"
    "}\n"
    "\n"
    "/* ==== Unsalted Streebog-256 (e430) ==== */\n"
    "\n"
    "/* Inline compact table probe for Metal */\n"
    "static bool probe_compact_inline(uint hx, uint hy, uint hz, uint hw,\n"
    "    device const uint *compact_fp, device const uint *compact_idx,\n"
    "    uint64_t compact_mask, uint max_probe, uint hash_data_count,\n"
    "    device const uchar *hash_data_buf, device const uint64_t *hash_data_off,\n"
    "    device const uint64_t *overflow_keys, device const uchar *overflow_hashes,\n"
    ...
    "            else {\n"
    "                for (int d = mid; d >= 0 && overflow_keys[d] == key; d--) {\n"
    ...
    "}\n"
    "\n"
    "kernel void streebog256_unsalted_batch(\n"
    "    device const uchar    *hexhashes  [[buffer(0)]],\n"
    "    device const ushort   *hexlens    [[buffer(1)]],\n"
    "    device const ushort   *unused2    [[buffer(2)]],\n"
    "    device const uchar    *mask_desc      [[buffer(3)]],\n"
    ...
    "    device const uchar    *hash_data_buf [[buffer(9)]],\n"
    ...
    "    device const uchar    *overflow_hashes [[buffer(15)]],\n"
    ...
    "\n"
    "    uchar hash[32];\n"
    "    streebog_hash_priv(hash, 32, pass, passlen);\n"
    "\n"
    "    uint hx, hy, hz, hw;\n"
    "    streebog_to_probe(hash, &hx, &hy, &hz, &hw);\n"
    "\n"
    "    /* Streebog-256: 8 hash words (32 bytes) */\n"
    "    uint hh[8];\n"
    "    for (int i = 0; i < 8; i++)\n"
    "        hh[i] = ((uint)hash[i*4]) | ((uint)hash[i*4+1] << 8)\n"
    "              | ((uint)hash[i*4+2] << 16) | ((uint)hash[i*4+3] << 24);\n"
    "\n"
    "    if (probe_compact_inline(hx, hy, hz, hw, compact_fp, compact_idx,\n"
    ...
    "}\n"
    "\n"
    "/* ==== Unsalted Streebog-512 (e431) ==== */\n"
    "kernel void streebog512_unsalted_batch(\n"
    "    device const uchar    *hexhashes  [[buffer(0)]],\n"
    "    device const ushort   *hexlens    [[buffer(1)]],\n"
    "    device const ushort   *unused2    [[buffer(2)]],\n"
    "    device const uchar    *mask_desc      [[buffer(3)]],\n"
    ...
    "\n"
    "    uchar hash[64];\n"
    "    streebog_hash_priv(hash, 64, pass, passlen);\n"
    "\n"
    "    uint hx, hy, hz, hw;\n"
    "    streebog_to_probe(hash, &hx, &hy, &hz, &hw);\n"
    "\n"
    "    /* Streebog-512: 16 hash words (64 bytes) */\n"
    "    uint hh[16];\n"
    "    for (int i = 0; i < 16; i++)\n"
    "        hh[i] = ((uint)hash[i*4]) | ((uint)hash[i*4+1] << 8)\n"
    "              | ((uint)hash[i*4+2] << 16) | ((uint)hash[i*4+3] << 24);\n"
    ...
    "}\n"
    "\n"
    "/* ==== HMAC-Streebog-256 key=$pass (e837, hashcat 11750) ==== */\n"
    "kernel void hmac_streebog256_kpass_batch(\n"
    "    device const uchar    *hexhashes  [[buffer(0)]],\n"
    "    device const ushort   *hexlens    [[buffer(1)]],\n"
    "    device const ushort   *unused2    [[buffer(2)]],\n"
    ...
    "    device const uchar *key = hexhashes + word_idx * 256;\n"
    "    uint soff = salt_offsets[salt_idx];\n"
    "    int mlen = salt_lens[salt_idx];\n"
    "    device const uchar *msg = salts + soff;\n"
    "\n"
    "    /* Prepare key block (64 bytes). If key > 64, hash it first. */\n"
    "    uchar key_block[64];\n"
    ...
    "        uchar kh[32];\n"
    "        uchar ktmp[256];\n"
    "        for (int i = 0; i < klen && i < 256; i++) ktmp[i] = key[i];\n"
    "        streebog_hash_priv(kh, 32, ktmp, klen);\n"
    "        for (int i = 0; i < 32; i++) key_block[i] = kh[i];\n"
    "        klen = 32;\n"
    "    } else {\n"
    "        for (int i = 0; i < klen; i++) key_block[i] = key[i];\n"
    "    }\n"
    "\n"
    "    /* Inner hash: streebog256((key ^ ipad) || salt) */\n"
    "    uchar ibuf[320];  /* 64 + 256 max */\n"
    "    for (int i = 0; i < 64; i++) ibuf[i] = key_block[i] ^ 0x36;\n"
    "    for (int i = 0; i < mlen && i < 256; i++) ibuf[64 + i] = msg[i];\n"
    "\n"
    "    uchar inner[32];\n"
    "    streebog_hash_buf(inner, 32, ibuf, 64 + mlen);\n"
    "\n"
    "    /* Outer hash: streebog256((key ^ opad) || inner_hash) */\n"
    "    uchar obuf[96];  /* 64 + 32 */\n"
    "    for (int i = 0; i < 64; i++) obuf[i] = key_block[i] ^ 0x5c;\n"
    "    for (int i = 0; i < 32; i++) obuf[64 + i] = inner[i];\n"
    "\n"
    "    uchar hash[32];\n"
    "    streebog_hash_buf(hash, 32, obuf, 96);\n"
    "\n"
    "    uint hx, hy, hz, hw;\n"
    "    streebog_to_probe(hash, &hx, &hy, &hz, &hw);\n"
    "\n"
    ...
    "}\n"
    "\n"
    "/* ==== HMAC-Streebog-256 key=$salt (e838, hashcat 11760) ==== */\n"
    "kernel void hmac_streebog256_ksalt_batch(\n"
    "    device const uchar    *hexhashes  [[buffer(0)]],\n"
    "    device const ushort   *hexlens    [[buffer(1)]],\n"
    "    device const ushort   *unused2    [[buffer(2)]],\n"
    "    device const uchar    *salts      [[buffer(3)]],\n"
    ...
    "    device const uchar *pass = hexhashes + word_idx * 256;\n"
    "    uint soff = salt_offsets[salt_idx];\n"
    "    int klen = salt_lens[salt_idx];\n"
    "    device const uchar *key = salts + soff;\n"
    "\n"
    "    /* Prepare key block from salt */\n"
    "    uchar key_block[64];\n"
    "    for (int i = 0; i < 64; i++) key_block[i] = 0;\n"
    "\n"
    "    if (klen > 64) {\n"
    ...
    "    }\n"
    "\n"
    "    /* Inner hash: streebog256((key ^ ipad) || password) */\n"
    "    uchar ibuf[320];\n"
    "    for (int i = 0; i < 64; i++) ibuf[i] = key_block[i] ^ 0x36;\n"
    "    for (int i = 0; i < plen && i < 256; i++) ibuf[64 + i] = pass[i];\n"
    "\n"
    "    uchar inner[32];\n"
    "    streebog_hash_buf(inner, 32, ibuf, 64 + plen);\n"
    "\n"
    "    /* Outer hash: streebog256((key ^ opad) || inner_hash) */\n"
    "    uchar obuf[96];\n"
    "    for (int i = 0; i < 64; i++) obuf[i] = key_block[i] ^ 0x5c;\n"
    ...
    "}\n"
    "\n"
    "/* ==== HMAC-Streebog-512 key=$pass (e839, hashcat 11850) ==== */\n"
    "kernel void hmac_streebog512_kpass_batch(\n"
    "    device const uchar    *hexhashes  [[buffer(0)]],\n"
    "    device const ushort   *hexlens    [[buffer(1)]],\n"
    "    device const ushort   *unused2    [[buffer(2)]],\n"
    "    device const uchar    *salts      [[buffer(3)]],\n"
    ...
    "    device const uchar *msg = salts + soff;\n"
    "\n"
    "    /* Prepare key block. If key > 64, hash with streebog-512 first. */\n"
    "    uchar key_block[64];\n"
    "    for (int i = 0; i < 64; i++) key_block[i] = 0;\n"
    "\n"
    "    if (klen > 64) {\n"
    ...
    "        uchar ktmp[256];\n"
    "        for (int i = 0; i < klen && i < 256; i++) ktmp[i] = key[i];\n"
    "        streebog_hash_priv(kh, 64, ktmp, klen);\n"
    "        for (int i = 0; i < 64; i++) key_block[i] = kh[i];\n"
    "        klen = 64;\n"
    "    } else {\n"
    "        for (int i = 0; i < klen; i++) key_block[i] = key[i];\n"
    "    }\n"
    "\n"
    "    /* Inner hash: streebog512((key ^ ipad) || salt) */\n"
    "    uchar ibuf[320];\n"
    "    for (int i = 0; i < 64; i++) ibuf[i] = key_block[i] ^ 0x36;\n"
    "    for (int i = 0; i < mlen && i < 256; i++) ibuf[64 + i] = msg[i];\n"
    "\n"
    "    uchar inner[64];\n"
    "    streebog_hash_buf(inner, 64, ibuf, 64 + mlen);\n"
    "\n"
    "    /* Outer hash: streebog512((key ^ opad) || inner_hash) */\n"
    "    uchar obuf[128];\n"
    "    for (int i = 0; i < 64; i++) obuf[i] = key_block[i] ^ 0x5c;\n"
    "    for (int i = 0; i < 64; i++) obuf[64 + i] = inner[i];\n"
    "\n"
    "    uchar hash[64];\n"
    "    streebog_hash_buf(hash, 64, obuf, 128);\n"
    "\n"
    "    uint hx, hy, hz, hw;\n"
    "    streebog_to_probe(hash, &hx, &hy, &hz, &hw);\n"
    "\n"
    ...
    "}\n"
    "\n"
    "/* ==== HMAC-Streebog-512 key=$salt (e840, hashcat 11860) ==== */\n"
    "kernel void hmac_streebog512_ksalt_batch(\n"
    "    device const uchar    *hexhashes  [[buffer(0)]],\n"
    "    device const ushort   *hexlens    [[buffer(1)]],\n"
    "    device const ushort   *unused2    [[buffer(2)]],\n"
    "    device const uchar    *salts      [[buffer(3)]],\n"
    ...
    "    }\n"
    "\n"
    "    /* Inner hash: streebog512((key ^ ipad) || password) */\n"
    "    uchar ibuf[320];\n"
    "    for (int i = 0; i < 64; i++) ibuf[i] = key_block[i] ^ 0x36;\n"
    "    for (int i = 0; i < plen && i < 256; i++) ibuf[64 + i] = pass[i];\n"
    "\n"
    "    uchar inner[64];\n"
    "    streebog_hash_buf(inner, 64, ibuf, 64 + plen);\n"
    "\n"
    "    /* Outer hash: streebog512((key ^ opad) || inner_hash) */\n"
    "    uchar obuf[128];\n"
    "    for (int i = 0; i < 64; i++) obuf[i] = key_block[i] ^ 0x5c;\n"
