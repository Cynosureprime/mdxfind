/* Auto-generated from gpu_wrlunsalted.cl -- do not edit */
static const char gpu_wrlunsalted_str[] =
    "/* gpu_wrlunsalted.cl — Pre-padded unsalted Whirlpool with mask expansion\n"
    " *\n"
    ...
    " *   Kernel byte-swaps to big-endian before Whirlpool compress.\n"
    " *\n"
    ...
    " */\n"
    "\n"
    "/* bswap32, bswap64 provided by gpu_common.cl */\n"
    "\n"
    ...
    "};\n"
    "\n"
    "/* Whirlpool S-Box: 8 sub-tables of 256 uint64 entries each */\n"
    ...
    "};\n"
    "\n"
    "/* Whirlpool WHIRLPOOL_OP macro equivalent */\n"
    ...
    "\n"
    "/* Whirlpool compress: single block with zero IV (for unsalted first-block-only hashing).\n"
    " * hash[] output = Miyaguchi-Preneel(0, block) = state_final ^ block ^ 0\n"
    " * Since hash starts as 0: K[0]=0, state[0]=block, final hash = state_final ^ block. */\n"
    "void whirlpool_compress(ulong *hash, ulong *block) {\n"
    "    ulong K[2][8];\n"
    "    ulong state[2][8];\n"
    "    int m = 0;\n"
    "\n"
    "    for (int i = 0; i < 8; i++) {\n"
    "        K[0][i] = 0;                /* hash is all zeros for first block */\n"
    "        state[0][i] = block[i];     /* state = block ^ hash = block ^ 0 */\n"
    "    }\n"
    "\n"
    "    for (int r = 0; r < 10; r++) {\n"
    ...
    "    }\n"
    "\n"
    "    /* Miyaguchi-Preneel: hash = old_hash ^ state ^ block.\n"
    "     * old_hash = 0, so hash = state ^ block.\n"
    "     * After 10 rounds with alternating m, final state is in state[0]. */\n"
    "    for (int i = 0; i < 8; i++)\n"
    "        hash[i] = state[0][i] ^ block[i];\n"
    "}\n"
    "\n"
    "/* General Whirlpool compress: hash = Miyaguchi-Preneel(cv, block) = cv ^ state ^ block */\n"
    "void whirlpool_compress_cv(ulong *hash, ulong *block, ulong *cv) {\n"
    "    ulong K[2][8];\n"
    "    ulong state[2][8];\n"
    "    int m = 0;\n"
    "\n"
    ...
    "}\n"
    "\n"
    "__kernel void wrl_unsalted_batch(\n"
    ...
    "    uint M32[16];\n"
    "    for (int i = 0; i < 16; i++) M32[i] = src[i];\n"
    "\n"
    "    /* Fill mask positions into M32[] -- operates on LE words (same as SHA1 kernel) */\n"
    ...
    "                pidx /= sz;\n"
    "                M32[i >> 2] = (M32[i >> 2] & ~(0xFFu << ((i & 3) << 3)))\n"
    "                             | ((uint)ch << ((i & 3) << 3));\n"
    "            }\n"
    "\n"
    "            if (n_app > 0) {\n"
    "                int total_len = M32[14] >> 3;\n"
    ...
    "                    aidx /= sz;\n"
    "                    int pos = app_start + i;\n"
    "                    M32[pos >> 2] = (M32[pos >> 2] & ~(0xFFu << ((pos & 3) << 3)))\n"
    "                                   | ((uint)ch << ((pos & 3) << 3));\n"
    "                }\n"
    "            }\n"
    "        } else {\n"
    ...
    "    }\n"
    "\n"
    "    /* Save bit-length from LE M32[14] before byte-swap */\n"
    "    uint bitlen = M32[14];\n"
    "\n"
    "    /* Reinterpret 16 x uint32 as 8 x uint64, then byte-swap to big-endian */\n"
    "    ulong M64[8];\n"
    "    for (int i = 0; i < 8; i++)\n"
    "        M64[i] = bswap64(((ulong)M32[i*2+1] << 32) | (ulong)M32[i*2]);\n"
    "\n"
    "    /* Fix up padding: Whirlpool stores 64-bit big-endian bit count at M64[7]\n"
    "     * (bytes 56-63). M64[6] should be zero (upper 64 bits of 256-bit length field,\n"
    "     * but Whirlpool only uses 64-bit length for short messages). */\n"
    "    M64[6] = 0;\n"
    "    M64[7] = (ulong)bitlen;\n"
    "\n"
    "    /* Whirlpool compress with zero IV */\n"
    "    ulong hash[8];\n"
    "    whirlpool_compress(hash, M64);\n"
    "\n"
    "    /* Byte-swap hash to LE, split all 8 uint64 into 16 uint32 for full output */\n"
    "    uint h[16];\n"
    "    for (int i = 0; i < 8; i++) {\n"
    "        ulong s = bswap64(hash[i]);\n"
    "        h[i*2]   = (uint)s;\n"
    "        h[i*2+1] = (uint)(s >> 32);\n"
    "    }\n"
    "\n"
    "    /* Probe compact table and iterate */\n"
    ...
    "        }\n"
    "        if (iter < max_iter) {\n"
    "            /* Hex-encode 16 LE h[] words into hex_buf[128] (128 hex chars).\n"
    "             * Whirlpool hash is stored LE in h[]. Each uint32 is LE byte order. */\n"
    "            uchar hex_buf[192];  /* 128 hex + padding block */\n"
    "            for (int i = 0; i < 16; i++) {\n"
    "                uint s = h[i];\n"
    ...
    "                hex_buf[i*8+0] = hi0+((hi0<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+1] = lo0+((lo0<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+2] = hi1+((hi1<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+3] = lo1+((lo1<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+4] = hi2+((hi2<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+5] = lo2+((lo2<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+6] = hi3+((hi3<10)?'0':('a'-10));\n"
    "                hex_buf[i*8+7] = lo3+((lo3<10)?'0':('a'-10));\n"
    "            }\n"
    "\n"
    "            /* Whirlpool(128-byte hex string): 3 blocks.\n"
    "             * Block 1: hex_buf[0..63], Block 2: hex_buf[64..127],\n"
    "             * Block 3: padding (0x80, zeros, 1024-bit count) */\n"
    "\n"
    "            /* Block 1: convert hex_buf[0..63] to 8 BE uint64 and compress with zero IV */\n"
    "            ulong blk[8];\n"
    "            for (int i = 0; i < 8; i++) {\n"
    "                int off = i*8;\n"
    ...
    "\n"
    "            /* Block 3: padding. 0x80 at byte 0, zeros, 128*8=1024 at last 32 bytes.\n"
    "             * Whirlpool stores 256-bit big-endian bit count in last 32 bytes of block. */\n"
    "            for (int i = 0; i < 8; i++) blk[i] = 0;\n"
    "            blk[0] = 0x8000000000000000UL;  /* 0x80 in first byte (BE) */\n"
    "            blk[7] = 1024UL;  /* bit count in last 8 bytes (low 64 bits) */\n"
    "            for (int i = 0; i < 8; i++) cv[i] = hash[i];\n"
    "            whirlpool_compress_cv(hash, blk, cv);\n"
    "\n"
    "            /* Convert BE hash back to LE uint32 for next iteration */\n"
    "            for (int i = 0; i < 8; i++) {\n"
    "                ulong s = bswap64(hash[i]);\n"
    "                h[i*2]   = (uint)s;\n"
    "                h[i*2+1] = (uint)(s >> 32);\n"
