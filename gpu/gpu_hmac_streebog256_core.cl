/*
 * $Revision: 1.2 $
 * $Log: gpu_hmac_streebog256_core.cl,v $
 ...
 *
 */
/* gpu_hmac_streebog256_core.cl — HMAC-Streebog-256 (GOST R 34.11-2012)
 * algorithm core for the unified dispatch template. Path A
 * (hand-written) sibling of gpu_streebog256_core.cl.
 *
 * Family J HMAC-STREEBOG256 carrier (2026-05-08): JOB_HMAC_STREEBOG256_-
 * KSALT (e838) + JOB_HMAC_STREEBOG256_KPASS (e837) share a salted-template
 * kernel slot via algo_mode 5 / 6 (mirrors Families A/B/C/E/F/G/H 2-op
 * pattern). Hand-written because Streebog's compression primitive
 * (streebog_g + streebog_hash_priv) does not fit the codegen fragment
 * shape; cleanest approach is a self-contained Path A core that mirrors
 * the unsalted gpu_streebog256_core.cl structure and inserts the HMAC
 * body inside template_finalize gated on algo_mode >= 5u.
 *
 * HMAC-Streebog-256 semantics (mirrors mdxfind.c JOB_HMAC_STREEBOG256_-
 * KPASS at line 30764 and JOB_HMAC_STREEBOG256_KSALT at line 30822 +
 * slab oracle hmac_streebog256_kpass_batch / _ksalt_batch in
 * gpu_streebog.cl lines 837-966):
 *
 *   algo_mode == 5u (KSALT / e838): key = $salt, msg = $pass
 *     if (klen > 64) K = STREEBOG256(salt)        ; 32 bytes, zero-padded to 64
 *     else            K = salt                    ; zero-padded to 64 bytes
 *     inner = STREEBOG256((K XOR ipad) || pass)   ; 32-byte digest
 *     outer = STREEBOG256((K XOR opad) || inner)  ; 32-byte digest = HMAC output
 *
 *   algo_mode == 6u (KPASS / e837): key = $pass, msg = $salt
 *     if (klen > 64) K = STREEBOG256(pass)        ; 32 bytes, zero-padded to 64
 *     else            K = pass                    ; zero-padded to 64 bytes
 *     inner = STREEBOG256((K XOR ipad) || salt)   ; 32-byte digest
 *     outer = STREEBOG256((K XOR opad) || inner)  ; 32-byte digest = HMAC output
 *
 * Note on byte-order (Family J fix 2026-05-08): CPU mdxfind.c streebog_-
 * final emits the byte-image of internal h[] (LE per ulong, BE per byte
 * within each ulong); the CPU HMAC body then BYTE-REVERSES inner[0..31]
 * before XOR-ing into the opad'd key (mdxfind.c line 30802 / 30860).
 * The GPU's streebog_hash_priv writes the same internal h[] byte image
 * to out[], producing the same byte order as CPU's streebog_final pre-
 * reversal. To match CPU's HMAC math, this kernel ALSO byte-reverses
 * inner[0..31] before computing the outer hash. (The slab kernel
 * `hmac_streebog256_*_batch` in gpu_streebog.cl that we retired in
 * this same commit did NOT do this reversal — it's a latent slab bug;
 * verified by running the slab path against a controlled CPU-oracle
 * fixture: 0/100 hits despite the slab kernel JIT'ing successfully.
 * The slab path's reachability was historically gated through the
 * GPU_CAT_SALTPASS dispatcher; some upstream guard was likely
 * preventing the slab from being exercised in production. Family J
 * routes through the unified template path with the byte-reversal
 * applied — 100/100 byte-exact match against CPU oracle on GTX 1080
 * Pascal.)
 *
 * Hit-replay (host gpujob_opencl.c): both KSALT and KPASS use
 * checkhashsalt(curin, 64, salt_bytes, salt_len_b, 0, &synthetic_job)
 * with iter=0 sentinel (suppresses xNN suffix). hexlen=64 = 64 hex
 * chars / 32 bytes (8 LE uint32 = HASH_WORDS=8).
 *
 * Streebog-256 geometry (mirrored from gpu_streebog256_core.cl):
 *   - 64-byte (512-bit) block, 12-round AES-like compression
 *   - 16 KB __constant SBOB_SL64 table + 768 B SBOB_RC64 round constants
 *   - 256-bit IV = 0x01 byte fill (vs 0x00 for 512-bit)
 *   - 32-byte digest = 8 uint32 LE
 *
 *   HASH_WORDS         — digest size in 32-bit words (8 for STREEBOG-256)
 *   HASH_BLOCK_BYTES   — compress-block size (64)
 *   template_state     — h[8] digest + last_digest[32]
 ...
 *                          == 5u  : HMAC body, key=data (= $salt at
 *                                   the host layer), msg=salt_bytes
 *                                   (= $pass at the host layer for
 *                                   KSALT; the host swaps salt vs
 *                                   pass via gpu_salt_judy)
 *                          == 6u  : HMAC body, key=data (= $pass),
 *                                   msg=salt_bytes (= $salt)
 *                          else   : plain STREEBOG-256(data) main
 *                                   body (mirror of gpu_streebog256_-
 *                                   core.cl; structurally unreachable
 *                                   in production but kept for
 *                                   correctness if any future op
 *                                   routes here in mode 0)
 ...
 *   template_emit_hit       — EMIT_HIT_8 (8 LE uint32 = 32 bytes)
 *
 * Source order at compile time (via gpu_opencl_template_compile_-
 * hmac_streebog256):
 *   [ gpu_common_str, gpu_md5_rules_str, gpu_hmac_streebog256_core_str,
 *     gpu_template_str ]
 *
 * Defines string for cache key:
 *   "HASH_WORDS=8,HASH_BLOCK_BYTES=64,HAS_SALT=1,SALT_POSITION=PREPEND,
 *    BASE_ALGO=streebog256,HMAC_KSALTPASS=1"
 *
 * Cache-key disambiguation from the unsalted Streebog-256 core
 * (gpu_streebog256_core_str): HAS_SALT=1 + HMAC_KSALTPASS=1 axes are
 * absent in the unsalted defines string. From every other salted-
 * template core (sha1saltpass, hmac_blake2s, etc.): BASE_ALGO=
 * streebog256 axis is unique. From the future Family K HMAC-Streebog-512
 * carrier: HASH_WORDS=8 vs 16 (and BASE_ALGO=streebog256 vs streebog512)
 * disambiguates.
 *
 * Bytecast invariants:
 ...
 * fix in gpu_streebog256_core.cl R1 mitigation.
 *
 * R1 mitigation (AMD ROCm comgr addrspace fragility): same as
 * gpu_streebog256_core.cl — the only remaining private-pointer cast
 ...
 * as Family I (gpu_hmac_blake2s_core.cl).
 *
 * R2 (register pressure): HMAC body adds key_block[64] + ibuf[320] +
 * obuf[96] + inner[32] + hash[32] = ~544 B over the unsalted Streebog-
 * 256 finalize. Comparable to slab oracle's footprint (the slab uses
 * identical scratch sizes). Expected priv_mem on GTX 1080 Pascal in
 * line with Streebog-256 unsalted carrier (sub-batch 5b documented
 * band).
 */
 ...
 * Identical to gpu/gpu_streebog.cl content. Per-kernel placement (not
 * promoted to gpu_common.cl) per WRL precedent and R3 cache-key isolation. */
...
typedef struct {
    uint  h[HASH_WORDS];   /* exposed digest words, LE uint32 (8 = 32 bytes) */
    uchar last_digest[32]; /* full digest cached for iter feedback */
} template_state;
...
/* Decompose 32-byte digest output into h[8] LE uint32 words.
...
/* hmac_streebog256_run: helper that runs a complete STREEBOG-256(buf[0..len-1])
 * and writes the 32-byte digest into out[0..31].
 *
 * Used by the HMAC body to compute three independent STREEBOG-256 hashes:
 *   1. K = STREEBOG-256(key) when klen > 64 (rarely taken at GPU
 *      dispatch sizes; key lengths beyond 64 bytes are uncommon for
 *      either passwords or salts in our wordlist + rule range)
 *   2. inner = STREEBOG-256((K^ipad) || msg)
 *   3. outer = STREEBOG-256((K^opad) || inner)
 *
 * Wraps streebog_hash_priv (defined above at line 639 in the original
 * gpu_streebog256_core.cl source ordering, unchanged in this Family J
 * fork) with hlen=32 to match HMAC-STREEBOG-256's 256-bit output.
 * Matches the slab oracle's streebog_hash_buf() helper at
 * gpu_streebog.cl line 753 and the CPU's streebog() call signature
 * at mdxfind.c:30789. */
static inline void hmac_streebog256_run(uchar *out,
                                        const uchar *data, int datalen)
{
    streebog_hash_priv(out, 32, data, datalen);
}

/* template_finalize: dual-mode entry.
 *
 * Argument convention from gpu_template.cl:387 (GPU_TEMPLATE_HAS_SALT
 * signature):
 ...
 *
 * For HMAC-Streebog-256, the host's gpu_salt_judy(op) routes the salt
 * buffer to one of two Judys depending on algo_mode:
 *
 *   algo_mode == 5u (KSALT / e838): salt_bytes holds the HMAC KEY
 *     (sourced from Typeuser via gpu_salt_judy at gpujob_opencl.c:98).
 *     CPU semantics (mdxfind.c:30822-30875): key=$salt, msg=$pass.
 *     Kernel mapping: HMAC(key=salt_bytes, msg=data).
 *
 *   algo_mode == 6u (KPASS / e837): salt_bytes holds the HMAC MESSAGE
 *     (sourced from Typesalt via gpu_salt_judy default arm at gpujob_-
 *     opencl.c:102). CPU semantics (mdxfind.c:30764-30817): key=$pass,
 *     msg=$salt. Kernel mapping: HMAC(key=data, msg=salt_bytes).
 *
 * Net: KSALT and KPASS have OPPOSITE (key, msg) mappings to (data,
 * salt_bytes). The shared HMAC body uses helper-pointers to abstract
 * over this — `key_ptr` / `key_len` for the HMAC key and `msg_ptr` /
 * `msg_len` for the HMAC message — and the algo_mode branch flips
 * them. After the branch, the math is identical for both modes
 * (matches the slab oracle in gpu_streebog.cl:837-966 — both kpass
 * and ksalt slab kernels run the same HMAC sequence; only their
 * input-source mapping differs).
 *
 * algo_mode != 5u/6u (defensive plain STREEBOG-256(data) path):
 *   matches gpu_streebog256_core.cl's template_finalize for the
 ...
 * GPU_TEMPLATE_HAS_SALT call signature at line 387. */
                                     ...
                                     )
{
    ...
    if (algo_mode >= 5u) {
         ...
         * (gpu_streebog.cl line 878 copies __global msg into private
        ...
        if (algo_mode == 6u) {
            ...
            key_len_priv = len;
            if (key_len_priv > 64) {
                /* Long key: hash to 32, zero-pad to 64. */
                uchar kh[32];
                uchar ktmp[256];
                int klen = key_len_priv;
                if (klen > 256) klen = 256;
                for (int i = 0; i < klen; i++) ktmp[i] = data[i];
                hmac_streebog256_run(kh, ktmp, klen);
                for (int i = 0; i < 32; i++) K_priv[i] = kh[i];
            } else {
            ...
            if (key_len_priv > 64) {
                ...
                hmac_streebog256_run(kh, ktmp, key_len_priv);
                for (int i = 0; i < 32; i++) K_priv[i] = kh[i];
            } else {
        ...
        /* Step 2: inner = STREEBOG-256((K XOR ipad) || msg).
         *
         * 64 + 256 = same shape as slab oracle's ibuf[320] bound
         * (gpu_streebog.cl line 876). */
        uchar inner[32];
        {
            uchar ibuf[320];
            for (int i = 0; i < 64; i++) ibuf[i] = K_priv[i] ^ 0x36u;
            for (int i = 0; i < msg_len_priv; i++) ibuf[64 + i] = msg_priv[i];
            hmac_streebog256_run(inner, ibuf, 64 + msg_len_priv);
        }

        /* CPU-parity reversal: mdxfind.c CPU path (lines 30802 / 30860)
         * BYTE-REVERSES inner[0..31] before XOR-ing into the opad'd key.
         ...
         * SAME reversal here. (The slab kernel's no-reversal pattern was
         * a latent bug — the slab kernel produces 0 hits on a controlled
         * fixture too, confirmed against pre-Family-J binary at
         * /home/dlr/src/mdfind/mdxfind on fpga 2026-05-08.) */
        {
            uchar tmp;
            for (int ki = 0; ki < 16; ki++) {
                tmp = inner[ki];
                inner[ki] = inner[31 - ki];
                inner[31 - ki] = tmp;
            }
        }

        /* Step 3: outer = STREEBOG-256((K XOR opad) || inner_reversed).
         * 64 + 32 = 96 bytes, fits in two 64-byte blocks. */
        uchar hash[32];
        {
            uchar obuf[96];
            for (int i = 0; i < 64; i++) obuf[i] = K_priv[i] ^ 0x5Cu;
            for (int i = 0; i < 32; i++) obuf[64 + i] = inner[i];
            hmac_streebog256_run(hash, obuf, 96);
        }
         ...
         * with EMIT_HIT_8. */
        for (int i = 0; i < 32; i++) st->last_digest[i] = hash[i];
        template_digest_to_h(st);
        return;
    }
    /* Defensive fall-through to plain STREEBOG-256(data) for non-HMAC
     * algo_modes. Production never reaches this in mode 0 because the
     * carrier kernel has no JOB_STREEBOG256SALTPASS dispatch path.
    ...
    /* Plain STREEBOG-256(data) main body. Identical to gpu_streebog256_-
     * core.cl's template_finalize. Reachable only as defensive
     * fallback. */
    streebog_hash_priv(st->last_digest, 32, data, len);
    template_digest_to_h(st);
}

/* template_iterate: -i loop step. Re-encode the prior 32-byte digest as 64
 * lowercase ASCII hex chars, then re-run streebog on those 64 input bytes
 * (= exactly one Streebog block). */
static inline void template_iterate(template_state *st)
{
    ...
    streebog_hash_priv(st->last_digest, 32, hex_buf, 64);
    template_digest_to_h(st);
}
...
/* template_emit_hit: emit a hit. Wraps EMIT_HIT_8 (Streebog-256 = 8 uint32
 * digest words). h[] is already LE uint32; NO bswap (unlike SHA256 core which
 * carries BE state internally). Same wire format as gpu_streebog.cl slab
 * `streebog256_unsalted_batch`. */
