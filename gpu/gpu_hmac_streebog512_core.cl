/*
 * $Revision: 1.2 $
 * $Log: gpu_hmac_streebog512_core.cl,v $
 ...
 *
 */
/* gpu_hmac_streebog512_core.cl — HMAC-Streebog-512 (GOST R 34.11-2012)
 * algorithm core for the unified dispatch template. Path A
 * (hand-written) sibling of gpu_streebog512_core.cl, mirrors Family J
 * (HMAC-STREEBOG-256) structure at HASH_WORDS=16.
 *
 * Family K HMAC-STREEBOG-512 carrier (2026-05-08): JOB_HMAC_STREEBOG512_-
 * KSALT (e840) + JOB_HMAC_STREEBOG512_KPASS (e839) share a salted-template
 * kernel slot via algo_mode 5 / 6 (mirrors Family J 2-op pattern). Hand-
 * written because Streebog's compression primitive (streebog_g +
 * streebog_hash_priv) does not fit the codegen fragment shape; cleanest
 * approach is a self-contained Path A core that mirrors the unsalted
 * gpu_streebog512_core.cl structure and inserts the HMAC body inside
 * template_finalize gated on algo_mode >= 5u.
 *
 * HMAC-Streebog-512 semantics (mirrors mdxfind.c JOB_HMAC_STREEBOG512_-
 * KPASS at line 30918 and JOB_HMAC_STREEBOG512_KSALT at line 30975 +
 * slab oracle hmac_streebog512_kpass_batch / _ksalt_batch in
 * gpu_streebog.cl lines 859-989):
 *
 *   algo_mode == 5u (KSALT / e840): key = $salt, msg = $pass
 *     if (klen > 64) K = STREEBOG512(salt)        ; 64 bytes
 *     else            K = salt                    ; zero-padded to 64 bytes
 *     inner = STREEBOG512((K XOR ipad) || pass)   ; 64-byte digest
 *     outer = STREEBOG512((K XOR opad) || inner)  ; 64-byte digest = HMAC output
 *
 *   algo_mode == 6u (KPASS / e839): key = $pass, msg = $salt
 *     if (klen > 64) K = STREEBOG512(pass)        ; 64 bytes
 *     else            K = pass                    ; zero-padded to 64 bytes
 *     inner = STREEBOG512((K XOR ipad) || salt)   ; 64-byte digest
 *     outer = STREEBOG512((K XOR opad) || inner)  ; 64-byte digest = HMAC output
 *
 * Note on byte-order (Family K mirrors Family J fix 2026-05-08): CPU
 * mdxfind.c streebog_final emits the byte-image of internal h[] (LE per
 * ulong, BE per byte within each ulong); the CPU HMAC body then BYTE-
 * REVERSES inner[0..63] before XOR-ing into the opad'd key (mdxfind.c
 * line 30955 / 31012). The GPU's streebog_hash_priv writes the same
 * internal h[] byte image to out[], producing the same byte order as
 * CPU's streebog_final pre-reversal. To match CPU's HMAC math, this
 * kernel ALSO byte-reverses inner[0..63] before computing the outer hash.
 * (The slab kernel `hmac_streebog512_*_batch` in gpu_streebog.cl that we
 * retired in this same commit did NOT do this reversal — it's a latent
 * slab bug, parallel to Family J's discovery on STREEBOG-256. Family K
 * applies the inner-reversal correctly for byte-exact CPU/GPU parity.)
 *
 * Hit-replay (host gpujob_opencl.c): both KSALT and KPASS use
 * checkhashsalt(curin, 128, salt_bytes, salt_len_b, 0, &synthetic_job)
 * with iter=0 sentinel (suppresses xNN suffix). hexlen=128 = 128 hex
 * chars / 64 bytes (16 LE uint32 = HASH_WORDS=16).
 *
 * Streebog-512 geometry (mirrored from gpu_streebog512_core.cl):
 *   - 64-byte (512-bit) block, 12-round AES-like compression
 *   - 16 KB __constant SBOB_SL64 table + 768 B SBOB_RC64 round constants
 *   - 512-bit IV = 0x00 byte fill (vs 0x01 for 256-bit)
 *   - 64-byte digest = 16 uint32 LE (full internal state h[8])
 *
 *   HASH_WORDS         — digest size in 32-bit words (16 for STREEBOG-512)
 *   HASH_BLOCK_BYTES   — compress-block size (64)
 *   template_state     — h[16] digest + last_digest[64]
 *   template_init      — no-op (streebog_hash_priv handles IV)
 *   template_transform — stub for template interface symmetry
 *   template_finalize  — branches on algo_mode:
 *                          == 5u  : HMAC body, key=salt_bytes, msg=data
 *                                   (KSALT: $salt is HMAC key, $pass is msg)
 *                          == 6u  : HMAC body, key=data, msg=salt_bytes
 *                                   (KPASS: $pass is HMAC key, $salt is msg)
 *                          else   : plain STREEBOG-512(data) main body
 *                                   (mirror of gpu_streebog512_core.cl;
 *                                   structurally unreachable in production
 *                                   but kept for correctness if any future
 *                                   op routes here in mode 0)
 *   template_iterate        — re-hash 128-char hex of digest (two-block)
 *   template_digest_compare — probe leading 16 bytes (h[0..3]) LE-direct
 *   template_emit_hit       — EMIT_HIT_16 (16 LE uint32 = 64 bytes)
 *
 * Source order at compile time (via gpu_opencl_template_compile_-
 * hmac_streebog512):
 *   [ gpu_common_str, gpu_md5_rules_str, gpu_hmac_streebog512_core_str,
 *     gpu_template_str ]
 *
 * Defines string for cache key:
 *   "HASH_WORDS=16,HASH_BLOCK_BYTES=64,HAS_SALT=1,SALT_POSITION=PREPEND,
 *    BASE_ALGO=streebog512,HMAC_KSALTPASS=1"
 *
 * Cache-key disambiguation from the unsalted Streebog-512 core
 * (gpu_streebog512_core_str): HAS_SALT=1 + HMAC_KSALTPASS=1 axes are
 * absent in the unsalted defines string. From every other salted-
 * template core (sha1saltpass, hmac_streebog256, etc.): BASE_ALGO=
 * streebog512 axis is unique. From Family J HMAC-Streebog-256 carrier:
 * HASH_WORDS=16 vs 8 (and BASE_ALGO=streebog512 vs streebog256)
 * disambiguates.
 *
 * Bytecast invariants:
 *   - Final st->h[0..15] is LITTLE-ENDIAN per uint32 — direct probe + emit,
 *     no bswap32. Matches the slab kernel's emit shape.
 *   - Wire format: 16 LE uint32 words written via EMIT_HIT_16.
 *   - Probe key uses st->h[0..3] (leading 16 bytes LE).
 *
 * RDNA4 (gfx1201) gotcha: SBOG_LPS macro uses shift-then-mask byte
 * extraction (not `uchar *tb = (uchar *)(src);` cast aliasing) per the
 * fix in gpu_streebog512_core.cl R1 mitigation.
 *
 * R1 mitigation (AMD ROCm comgr addrspace fragility): same as
 * gpu_streebog512_core.cl — the only remaining private-pointer cast
 * is at the n-XOR site in streebog_g and the IV/output uchar fills.
 * The HMAC body adds NO new addrspace casts; all HMAC scratch buffers
 * (key_block, ibuf, obuf, inner, hash) are __private and read salt_-
 * bytes (__global) into private staging before hashing — same shape
 * as Family J (gpu_hmac_streebog256_core.cl).
 *
 * R2 (register pressure): HMAC body adds K_priv[64] + msg_priv[256] +
 * ibuf[320] + obuf[128] + inner[64] + hash[64] = ~896 B over the
 * unsalted Streebog-512 finalize. Larger than Family J's HMAC-STREEBOG-
 * 256 footprint by ~+352 B (inner/hash are 64 bytes vs 32; obuf is 128
 * bytes vs 96). Comparable to slab oracle's footprint (the slab uses
 * identical scratch sizes for STREEBOG-512 HMAC). Expected priv_mem on
 * GTX 1080 Pascal in line with Family J's reading + ~+352 B.
 */
...
/* Streebog tables: SBOB_SL64[8][256] (16 KB) + SBOB_RC64[12][8] (768 B). */

__constant ulong SBOB_SL64[8][256] = {
  {
...
__constant ulong SBOB_RC64[12][8] = {
  {0xe9daca1eda5b08b1UL,0x1f7c65c0812fcbebUL,0x16d0452e43766a2fUL,0xfcc485758db84e71UL,0x0169679291e07c4bUL,0x15d360a4082a42a2UL,0x234d74cc36747605UL,0x0745a6f2596580ddUL},
  {0x1a2f9da98ab5a36fUL,0xd7b5700f469de34fUL,0x982b230a72eafef3UL,0x3101b5160f5ed561UL,0x5899d6126b17b59aUL,0xcaa70adbc261b55cUL,0x56cdcbd71ba2dd55UL,0xb79bb121700479e6UL},
  {0xc72fce2bacdc74f5UL,0x35843d6a28fc390aUL,0x8b1f9c525f5ef106UL,0x7b7b29b11475eaf2UL,0xb19e3590e40fe2d3UL,0x09db6260373ac9c1UL,0x31db7a8643f4b6c2UL,0xb20aba0af5961e99UL},
  {0xd26615e8b3df1fefUL,0xdde4715da0e148f9UL,0x7d3c5c337e858e48UL,0x3f355e68ad1c729dUL,0x75d603ed822cd7a9UL,0xbe0352933313b7d8UL,0xf137e893a1ea5334UL,0x2ed1e384bcbe0c22UL},
  {0x994747adac6bea4bUL,0x6323a96c0c413f9aUL,0x4a1086161f1c157fUL,0xbdff0f80d7359e35UL,0xa3f53a254717cdbfUL,0x161a2723b700ffdfUL,0xf563eaa97ea2567aUL,0x57fe6c7cfd581760UL},
  {0xd9d33a1daeae4faeUL,0xc039307a3bc3a46fUL,0x6ca44251f9c4662dUL,0xc68ef09ab49a7f18UL,0xb4b79a1cb7a6facfUL,0xb6c6bec2661ff20aUL,0x354f903672c571bfUL,0x6e7d64467a4068faUL},
  {0xecc5aaee160ec7f4UL,0x540924bffe86ac51UL,0xc987bfe6c7c69e39UL,0xc9937a19333e47d3UL,0x372c822dc5ab9209UL,0x04054a2883694706UL,0xf34a3ca24c451735UL,0x93d4143a4d568688UL},
  {0xa7c9934d425b1f9bUL,0x41416e0c02aae703UL,0x1ede369c71f8b74eUL,0x9ac4db4d3b44b489UL,0x90069b92cb2b89f4UL,0x2fc4a5d12b8dd169UL,0xd9a8515935c2ac36UL,0x1ee702bfd40d7fa4UL},
  {0x9b223116545a8f37UL,0xde5f16ecd89a4c94UL,0x244289251b3a7d3aUL,0x84090de0b755d93cUL,0xb1ceb2db0b440a80UL,0x549c07a69a8a2b7bUL,0x602a1fcb92dc380eUL,0xdb5a238351446172UL},
  {0x526f0580a6debeabUL,0xf3f3e4b248e52a38UL,0xdb788aff1ce74189UL,0x0361331b8ae1ff1fUL,0x4b3369af0267e79fUL,0xf452763b306c1e7aUL,0xc3b63b15d1fa9836UL,0xed9c4598fbc7b474UL},
  {0xfb89c8efd09ecd7bUL,0x94fe5a63cdc60230UL,0x6107abebbb6bfad8UL,0x7966841421800120UL,0xcab948eaef711d8aUL,0x986e477d1dcdbaefUL,0x5dd86fc04a59a2deUL,0x1b2df381cda4ca6bUL},
  {0xba3116f167e78e37UL,0x7ab14904b08013d2UL,0x771ddfbc323ca4cdUL,0x9b9f2130d41220f8UL,0x86cc91189def805dUL,0x5228e188aaa41de7UL,0x991bb2d9d517f4faUL,0x20d71bf14a92bc48UL}
};
#define SBOG_LPS(dst, src) do { \
    for (int _i = 0; _i < 8; _i++) { \
        int _sh = _i * 8; \
        (dst)[_i] = SBOB_SL64[0][((src)[0] >> _sh) & 0xff] ^ \
                    SBOB_SL64[1][((src)[1] >> _sh) & 0xff] ^ \
                    SBOB_SL64[2][((src)[2] >> _sh) & 0xff] ^ \
                    SBOB_SL64[3][((src)[3] >> _sh) & 0xff] ^ \
                    SBOB_SL64[4][((src)[4] >> _sh) & 0xff] ^ \
                    SBOB_SL64[5][((src)[5] >> _sh) & 0xff] ^ \
                    SBOB_SL64[6][((src)[6] >> _sh) & 0xff] ^ \
                    SBOB_SL64[7][((src)[7] >> _sh) & 0xff]; \
    } \
} while(0)

/* ---- The "g" compression function ---- */
void streebog_g(ulong h[8], const ulong m[8], ulong n) {
    ulong k[8], s[8], t[8];
    ...
    for (int r = 0; r < 12; r++) {
        /* s = LPS(s ^ k) */
        for (int i = 0; i < 8; i++) t[i] = s[i] ^ k[i];
        SBOG_LPS(s, t);
        /* k = LPS(k ^ rc[r]) */
        for (int i = 0; i < 8; i++) t[i] = k[i] ^ SBOB_RC64[r][i];
        SBOG_LPS(k, t);
    }
...
void streebog_hash_priv(uchar *out, int hlen, const uchar *data, int datalen) {
    ...
    if (hlen == 32) {
        ...
        for (int i = 0; i < 64; i++) out[i] = hb[i];
    }
}

/* ---- Per-lane template state ---- */
typedef struct {
    uint  h[HASH_WORDS];   /* exposed digest words, LE uint32 (16 = 64 bytes) */
    uchar last_digest[64]; /* full digest cached for iter feedback */
} template_state;

static inline void template_init(template_state *st) {
    /* No-op. streebog_hash_priv handles its own IV setup based on hlen. */
    (void)st;
}

/* Stub for template interface symmetry. */
static inline void template_transform(template_state *st, const uchar *block) {
    (void)st;
    (void)block;
}

/* Decompose 64-byte digest output into h[16] LE uint32 words.
 * Streebog digest bytes are already in LE order (the internal h[] is filled
 * via byte writes from the streebog_g state); just pack 4 bytes -> uint32. */
static inline void template_digest_to_h(template_state *st) {
    for (int i = 0; i < 16; i++) {
        st->h[i] = (uint)st->last_digest[i*4]
                 | ((uint)st->last_digest[i*4 + 1] << 8)
                 | ((uint)st->last_digest[i*4 + 2] << 16)
                 | ((uint)st->last_digest[i*4 + 3] << 24);
    }
}

/* hmac_streebog512_run: helper that runs a complete STREEBOG-512(buf[0..len-1])
 * and writes the 64-byte digest into out[0..63].
 *
 * Used by the HMAC body to compute three independent STREEBOG-512 hashes:
 *   1. K = STREEBOG-512(key) when klen > 64 (rarely taken at GPU
 *      dispatch sizes; key lengths beyond 64 bytes are uncommon for
 *      either passwords or salts in our wordlist + rule range)
 *   2. inner = STREEBOG-512((K^ipad) || msg)
 *   3. outer = STREEBOG-512((K^opad) || inner_reversed)
 *
 * Wraps streebog_hash_priv with hlen=64 to match HMAC-STREEBOG-512's
 * 512-bit output. Matches the slab oracle's streebog_hash_buf() helper at
 * gpu_streebog.cl line 753 and the CPU's streebog() call signature
 * at mdxfind.c:30943. */
static inline void hmac_streebog512_run(uchar *out,
                                        const uchar *data, int datalen)
{
    streebog_hash_priv(out, 64, data, datalen);
}

/* template_finalize: dual-mode entry.
 *
 * Argument convention from gpu_template.cl GPU_TEMPLATE_HAS_SALT signature:
 *   data       = the (rule-applied) candidate password word
 *   salt_bytes = the salt-axis buffer (uploaded once via
 *                gpu_opencl_set_salts; resolved per-op via
 *                gpu_salt_judy() at the host pack site).
 *
 * For HMAC-Streebog-512, BOTH KSALT and KPASS have TYPEOPT_NEEDSALT
 * (mdxfind.c lines 7057-7058); gpu_salt_judy() routes both to the
 * Typesalt default arm (gpu/gpujob_opencl.c lines 96-109). Same routing
 * pattern as Family J HMAC-STREEBOG-256.
 *
 *   algo_mode == 5u (KSALT / e840): salt_bytes holds the HMAC KEY
 *     (sourced from Typesalt[op] which holds the salt-data per the
 *     structured-hash loader). CPU semantics (mdxfind.c:30975-31029):
 *     key=$salt, msg=$pass. Kernel mapping: HMAC(key=salt_bytes, msg=data).
 *
 *   algo_mode == 6u (KPASS / e839): salt_bytes holds the HMAC MESSAGE.
 *     CPU semantics (mdxfind.c:30918-30972): key=$pass, msg=$salt.
 *     Kernel mapping: HMAC(key=data, msg=salt_bytes).
 *
 * Net: KSALT and KPASS have OPPOSITE (key, msg) mappings to (data,
 * salt_bytes). The shared HMAC body uses the algo_mode branch to swap
 * the key vs message source. After the branch, the math is identical
 * for both modes (matches the slab oracle in gpu_streebog.cl:860-989 —
 * both kpass and ksalt slab kernels run the same HMAC sequence; only
 * their input-source mapping differs).
 *
 * algo_mode != 5u/6u (defensive plain STREEBOG-512(data) path):
 *   matches gpu_streebog512_core.cl's template_finalize for the
 *   unsalted case. Structurally unreachable in production (mode-0 of
 *   this kernel is dead code per the carrier-kernel design) but kept
 *   for correctness parity with the unsalted sibling.
 *
 * Salt bytes for the HMAC come via the standard salted-template trio
 * uploaded by gpu_opencl_set_salts(): __global salt_buf (raw salt
 * bytes), __global salt_off (per-salt byte offset), __global
 * salt_lens (per-salt length). gpu_template.cl threads `salt_buf +
 * salt_off[i]` and `salt_lens[i]` into template_finalize per the
 * GPU_TEMPLATE_HAS_SALT call signature. */
...
#ifdef GPU_TEMPLATE_HAS_SALT
    if (algo_mode >= 5u) {
        /* Stage the message into a private buffer up-front. salt_bytes
         * is __global; private staging matches the slab oracle pattern
         * (gpu_streebog.cl line 901 copies __global msg into private
         * ibuf). For KPASS, msg comes from salt_bytes; for KSALT, msg
         * comes from data (already private — direct copy is a no-op
         * for the message arm). Net: one private staging buffer holds
         * the message in either mode.
         *
         * R1 mitigation: no __private/__global addrspace mixing inside
         * streebog_hash_priv; the staged buffers are private, matching
         * every other HMAC body in the template family. */
        uchar K_priv[64];               /* HMAC key, zero-padded to 64 */
        uchar msg_priv[256];            /* private msg staging */
        int   key_len_priv = 0;
        int   msg_len_priv = 0;

        for (int i = 0; i < 64; i++) K_priv[i] = 0;

        if (algo_mode == 6u) {
            /* KPASS: key=data (password), msg=salt_bytes. */
            key_len_priv = len;
            if (key_len_priv > 64) {
                /* Long key: hash to 64, write to K_priv (full 64 bytes). */
                uchar kh[64];
                uchar ktmp[256];
                int klen = key_len_priv;
                if (klen > 256) klen = 256;
                for (int i = 0; i < klen; i++) ktmp[i] = data[i];
                hmac_streebog512_run(kh, ktmp, klen);
                for (int i = 0; i < 64; i++) K_priv[i] = kh[i];
            } else {
                for (int i = 0; i < key_len_priv; i++) K_priv[i] = data[i];
            }
            uint slen = salt_len;
            if (slen > 256u) slen = 256u;
            for (uint i = 0; i < slen; i++) msg_priv[i] = salt_bytes[i];
            msg_len_priv = (int)slen;
        } else {
            /* algo_mode == 5u, KSALT: key=salt_bytes, msg=data. */
            uint slen = salt_len;
            if (slen > 256u) slen = 256u;
            key_len_priv = (int)slen;
            if (key_len_priv > 64) {
                /* Long key: stage __global salt into private, hash, pad. */
                uchar kh[64];
                uchar ktmp[256];
                for (int i = 0; i < key_len_priv; i++) ktmp[i] = salt_bytes[i];
                hmac_streebog512_run(kh, ktmp, key_len_priv);
                for (int i = 0; i < 64; i++) K_priv[i] = kh[i];
            } else {
                for (int i = 0; i < key_len_priv; i++) K_priv[i] = salt_bytes[i];
            }
            int dlen = len;
            if (dlen > 256) dlen = 256;
            for (int i = 0; i < dlen; i++) msg_priv[i] = data[i];
            msg_len_priv = dlen;
        }

        /* Step 2: inner = STREEBOG-512((K XOR ipad) || msg).
         *
         * 64 + 256 = same shape as slab oracle's ibuf[320] bound
         * (gpu_streebog.cl line 899). */
        uchar inner[64];
        {
            uchar ibuf[320];
            for (int i = 0; i < 64; i++) ibuf[i] = K_priv[i] ^ 0x36u;
            for (int i = 0; i < msg_len_priv; i++) ibuf[64 + i] = msg_priv[i];
            hmac_streebog512_run(inner, ibuf, 64 + msg_len_priv);
        }

        /* CPU-parity reversal: mdxfind.c CPU path (lines 30955 / 31012)
         * BYTE-REVERSES inner[0..63] before XOR-ing into the opad'd key.
         * The CPU's streebog_final outputs the byte-image of the internal
         * h[] state which, after Streebog's BE-internal accumulation,
         * needs reversal to align with the wire-format BE the CPU then
         * checks against (curin.h vs hex). The slab GPU streebog_hash_-
         * priv writes bytes in the same byte-image order as CPU's
         * streebog_final, so to match CPU's HMAC math we must do the
         * SAME reversal here. (The slab kernel's no-reversal pattern
         * was a latent bug — same shape as Family J's STREEBOG-256
         * discovery; the slab path was historically gated through
         * GPU_CAT_SALTPASS + an upstream guard that prevented the
         * slab from being exercised in production.) */
        {
            uchar tmp;
            for (int ki = 0; ki < 32; ki++) {
                tmp = inner[ki];
                inner[ki] = inner[63 - ki];
                inner[63 - ki] = tmp;
            }
        }

        /* Step 3: outer = STREEBOG-512((K XOR opad) || inner_reversed).
         * 64 + 64 = 128 bytes, fits in two 64-byte blocks. */
        uchar hash[64];
        {
            uchar obuf[128];
            for (int i = 0; i < 64; i++) obuf[i] = K_priv[i] ^ 0x5Cu;
            for (int i = 0; i < 64; i++) obuf[64 + i] = inner[i];
            hmac_streebog512_run(hash, obuf, 128);
        }

        /* Install HMAC digest into st->last_digest[] then decompose
         * into st->h[] LE-direct via the shared helper. Same shape as
         * the unsalted finalize below — keeps wire format byte-exact
         * with EMIT_HIT_16. */
        for (int i = 0; i < 64; i++) st->last_digest[i] = hash[i];
        template_digest_to_h(st);
        return;
    }
    /* Defensive fall-through to plain STREEBOG-512(data) for non-HMAC
     * algo_modes. Production never reaches this in mode 0 because the
     * carrier kernel has no JOB_STREEBOG512SALTPASS dispatch path.
     * salt_bytes / salt_len are unused in this path; OpenCL's __global
     * pointer cannot be cast to (void) cleanly across all compilers,
     * so we let the optimizer elide them. */
#endif

    /* Plain STREEBOG-512(data) main body. Identical to gpu_streebog512_-
     * core.cl's template_finalize. Reachable only as defensive
     * fallback. */
    streebog_hash_priv(st->last_digest, 64, data, len);
    template_digest_to_h(st);
}

/* template_iterate: -i loop step. Re-encode the prior 64-byte digest as 128
 * lowercase ASCII hex chars, then re-run streebog on those 128 input bytes
 * (= exactly two Streebog blocks). */
static inline void template_iterate(template_state *st)
{
    uchar hex_buf[128];
    for (int i = 0; i < 64; i++) {
        uint b = st->last_digest[i];
        uint hi = (b >> 4) & 0xf, lo = b & 0xf;
        hex_buf[i*2]     = (uchar)(hi + ((hi < 10u) ? '0' : ('a' - 10)));
        hex_buf[i*2 + 1] = (uchar)(lo + ((lo < 10u) ? '0' : ('a' - 10)));
    }
    streebog_hash_priv(st->last_digest, 64, hex_buf, 128);
    template_digest_to_h(st);
}

/* template_digest_compare: probe the compact table with the leading 16
 * bytes of the digest. h[] is already LE uint32 (template_digest_to_h
 * builds it byte-by-byte from last_digest[]); no bswap needed. */
...
/* template_emit_hit: emit a hit. Wraps EMIT_HIT_16 (Streebog-512 = 16 uint32
 * digest words). h[] is already LE uint32; NO bswap (unlike SHA512 core which
 * carries BE state internally). Same wire format as gpu_streebog.cl slab
 * `hmac_streebog512_*_batch` kernels. */
#define template_emit_hit(hits, hit_count, max_hits, st, widx, sidx, iter)     do {         EMIT_HIT_16((hits), (hit_count), (max_hits),                     (widx), (sidx), (iter), (st)->h)     } while (0)

/* B3 dedup+overflow-aware variant. */
#define template_emit_hit_or_overflow(hits, hit_count, max_hits, st, widx, sidx, iter,                                       hashes_shown, matched_idx, dedup_mask,                                       ovr_set, ovr_gid, lane_gid)     do {         EMIT_HIT_16_DEDUP_OR_OVERFLOW((hits), (hit_count), (max_hits),                     (widx), (sidx), (iter), (st)->h,                     (hashes_shown), (matched_idx), (dedup_mask),                     (ovr_set), (ovr_gid), (lane_gid));     } while (0)
