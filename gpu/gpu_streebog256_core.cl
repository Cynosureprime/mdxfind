 ...
 *
 */
/* gpu_streebog256_core.cl — Streebog-256 (GOST R 34.11-2012, -m e430)
 * algorithm extension functions for the generic dispatch template
 * (Memo B Phase B5 sub-batch 5b retry).
 *
 * Streebog (also known as GOST R 34.11-2012) is a Merkle-Damgard hash
 * with a 64-byte (512-bit) block, 12-round AES-like compression, and
 * a 16 KB __constant SBOB_SL64 table. The 256-bit and 512-bit variants
 * differ only in IV (0x01 fill vs 0x00 fill) and digest output truncation.
 *
 * RDNA4 (gfx1201) gotcha:
 *   The SBOG_LPS macro is REWRITTEN here (and in the slab kernel
 *   gpu/gpu_streebog.cl, rev 1.5+) to use shift-then-mask byte extraction
 *   instead of the original `uchar *tb = (uchar *)(src);` cast aliasing.
 *   The original byte-pointer aliasing produces wavefront-coherent codegen
 *   breakage on RDNA4 in the unified-template path (4/100 lanes correct,
 *   positions 1/33/65/97 only). WRL_OP uses the same shift-then-mask
 *   pattern at 16 KB __constant size and works 100/100 on RDNA4
 *   (sub-batch 6.5). Byte-identical to the original cast on LE host
 *   targets (all CPU/GPU we care about).
 *
 * State carried in template_state:
 *   uint  h[8]            — exposed digest words, LE uint32 (32 bytes total)
 *   uchar last_digest[32] — full 32-byte digest cached for iter feedback
 *
 * Cache key (R3): defines_str = "HASH_WORDS=8,HASH_BLOCK_BYTES=64".
 * Distinct cache entry guaranteed by source-text hash difference vs
 * Streebog-512 sibling.
 *
 * R1 mitigation (AMD ROCm comgr addrspace fragility): the only remaining
 * private-pointer cast in streebog_g/streebog_hash_priv is at the n-XOR
 * site (mb byte stores) and the IV/output uchar fills. Those are linear
 * write patterns, not table-indexed reads — different machinery from
 * SBOG_LPS. If gfx1201 still fails after this rewrite, those sites are
 * the next candidates.
 */
...
/* template_finalize: hash input bytes via streebog_hash_priv with hlen=32.
 * Decompose 32-byte digest into h[8] LE for emit + cache last_digest[] for
 * iter feedback. */
