 ...
 * Revision 1.21  2026/08/30 15:23:24  dlr
 * Add a shape invariant to hx_emit_unsalted_single_{opencl,metal}. Both emitters produce a one-shot hash(pass) kernel and read only code[1], silently ignoring anything past it. The RAW family e33 MD5RAW, e34 SHA1RAW and e36 SHA256RAW is ncode 4 (PUSH_VAR, CALL role 1, CALL role 0, HALT) because mdxfind.c 1.549 restored its base to X(X_bin(pass)); routing one of those here would emit a single-hash kernel and return a wrong digest with no diagnostic. hx_detect_pattern gates on ncode 3 so it cannot happen today, but unsalted_job_table in hx_emit_primitives.c still lists those three as unsalted-single, so the table and the generated bytecode disagree and only the detector holds them apart. Now FATALs on ncode not 3 or code[2] not OP_HALT, matching the existing FATAL style in these files. Turns a silent wrong answer into a loud failure.
 *
 ...
 * five helpers per byte-exact chain fix; see file header Revision 1.21  2026/08/30 15:23:24  dlr
 * five helpers per byte-exact chain fix; see file header Add a shape invariant to hx_emit_unsalted_single_{opencl,metal}. Both emitters produce a one-shot hash(pass) kernel and read only code[1], silently ignoring anything past it. The RAW family e33 MD5RAW, e34 SHA1RAW and e36 SHA256RAW is ncode 4 (PUSH_VAR, CALL role 1, CALL role 0, HALT) because mdxfind.c 1.549 restored its base to X(X_bin(pass)); routing one of those here would emit a single-hash kernel and return a wrong digest with no diagnostic. hx_detect_pattern gates on ncode 3 so it cannot happen today, but unsalted_job_table in hx_emit_primitives.c still lists those three as unsalted-single, so the table and the generated bytecode disagree and only the detector holds them apart. Now FATALs on ncode not 3 or code[2] not OP_HALT, matching the existing FATAL style in these files. Turns a silent wrong answer into a loud failure.
    ...
    const struct hx_spec_entry *entry)
{
    ...
    /* Shape invariant. This emitter produces a ONE-SHOT hash(pass) kernel and
     * reads only code[1]; it silently ignores anything past it. The RAW family
     * (e33 MD5RAW, e34 SHA1RAW, e36 SHA256RAW) is ncode==4 -- PUSH_VAR, CALL
     * role=1, CALL role=0, HALT -- because mdxfind.c 1.549 restored its base to
     * X(X_bin(pass)). Routing one of those here would emit a single-hash kernel
     * and return a WRONG digest with no diagnostic. hx_detect_pattern() gates on
     * ncode==3 so it cannot happen today, but unsalted_job_table in
     * hx_emit_primitives.c still lists e33/e34/e36 as unsalted-single, so the
     * table and the bytecode disagree and only the detector holds them apart.
     * Fail loudly rather than emit a wrong kernel. */
    if (prog->ncode != 3 || prog->code[2].op != OP_HALT) {
        fprintf(stderr,
            "FATAL: %s:%d hx_emit_unsalted_single_metal: e%d %s is not a one-shot shape "
            "(ncode=%d, expected 3 with code[2]=OP_HALT). This emitter would "
            "silently drop the remaining ops and produce a wrong digest.\n",
            __FILE__, __LINE__, entry->job_enum,
            entry->name ? entry->name : "(noname)", prog->ncode);
        return -1;
    }
